...
gg1<-ggplot(data=trainp, aes(x=sex, fill = as.factor(dropout)))+  geom_bar() + facet_grid (~race)+ theme(legend.position="top")+ labs(fill="Dropout") #+geom_bar(aes(fill=dropout), position="stack")

```

```{r white, echo=FALSE, fig.show="hold", fig.cap="Visualization of dropout data by race and gender. The white group has by far the most students and is therefore set apart from the minority races.", fig.width = 6, fig.height = 4}
trainp<-train[race=="White"]
trainp<-trainp[,sex:=ifelse(sex=="Male", "M", "F")]
gg2<-ggplot(data=trainp, aes(x=sex, fill = as.factor(dropout)))+  geom_bar() + facet_grid (~race)+ theme(legend.position = "none") #+geom_bar(aes(fill=dropout), position="stack")                       
...
dat<-data.frame(Parameter=c("NA","Female", "Asian", "Black", "Hispanic", "Native American", "Other Minorities"), Odds_Ratio=oddsrat)[-1,]
setnames(dat, "Coefficient", "Odds Ratio")
knitr::kable(
  dat,
  digits = 3,
...
knitr::kable(effsize, caption = "Effect Sizes for the Effects of Race and Gender on Dropout Rate") %>%
kableExtra::kable_styling(
bootstrap_options = c("striped", "condensed"),
font_size = 12, latex_options = "HOLD_position")

```
McFadden's and Coxsnell's statistics suggest that the model explains .3% of the variation while Nagelkerke's suggests that the model explains .5% of the variation. While this may seem like a very small amount, keep in mind that we are attempting to predict dropout rates based solely on race and gender. We would not expect, and certainly not hope, that these factors would explain a large amount of variation in dropout rates. It is also worth noting that using a likelihood ratio test and chi$^2$ statistic, we get a p value of less than .0001, which implies that our model should still be considered significant relative to the null model.
